...
- **Gestió d'aula:** cada alumne/a fa parlar **les seves dues micro:bit** i usa un **`group` de ràdio propi — el seu número de llista**, per evitar interferències amb la resta; l'ESP32 és **opcional/avançat**. Reforça les diferències de llenguatge amb `Classes/SA0/` (Part B).
...
- **Repte A (estació meteorològica connectada)** i **repte B (alerta a distància)** → la telemetria del rover: el mateix binomi emissor/receptor per ràdio és com el rover envia distància i estat des de la micro:bit de bord cap a la micro:bit base amb pantalla OLED.
