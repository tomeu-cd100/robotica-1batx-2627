# 00 · Guia de la defensa oral (escala de progressió del curs)

> **Per a qui és?** Docent i alumnat. La **defensa oral** s'avalua amb la rúbrica **R4** des de la SA1, i culmina a la SA9 amb una defensa completa. Perquè el salt no es faci de cop, l'exigència **creix per graons planificats** — com el codi (PRIMM → full en blanc), la comunicació també té la seva retirada de bastida.

## L'escala (què s'exigeix a cada trimestre)

| Trimestre | Format | Durada | Contingut exigit | On es practica |
|---|---|---|---|---|
| **T1** | Mini-defensa a peu de taula | **1'** + 1 pregunta | *Què fa* el sistema i *on s'usaria* al món real. | Productes de SA1–SA3. |
| **T2** | Defensa amb guió | **2–3'** + 2 preguntes | Problema → solució → **una decisió tècnica justificada** ("ho vam fer així i no aixà perquè…"). | Productes de SA4–SA6. |
| **T3 / SA9** | Defensa completa | **5'** + torn de preguntes | Guió sencer + **demostració en directe** + respondre sense llegir el dossier. | SA7 (demostració) i SA9 (defensa final). |

> La pregunta del docent a T1 és sempre amable i concreta ("què passa si tapes el sensor?"). A T2 una de les dues preguntes apunta a la **decisió tècnica**. A T3 el torn és obert (docent + públic).

## El guió de defensa (l'esquelet de sempre)

1. **Problema** — què havia de resoldre el sistema (1 frase).
2. **Solució** — què fa i com (el model **entrada → procés → sortida** és el millor mapa: *què percep, què decideix, què fa*).
3. **Decisió tècnica** — una tria justificada (llindar, histèresi, funció, sensor…): *per què així i no d'una altra manera*.
4. **Demostració** — el requisit principal funcionant.
5. *(T3)* **Preguntes** — respondre amb criteri; "no ho sé, però ho comprovaria així" és una resposta vàlida.

## Errors típics (i com evitar-los)

| Error | Antídot |
|---|---|
| Llegir el dossier o la pantalla | El guió són **5 paraules clau** en un post-it, no un text. |
| Explicar el codi línia a línia | Ningú no ho segueix: explica **la decisió**, no la sintaxi. |
| Callar quan la demo falla | **DEPURA en veu alta:** "esperàvem X, passa Y, la meva hipòtesi és Z". Demostra més competència que una demo perfecta. |
| Que sempre parli el mateix | A T2 i T3, **totes les persones de l'equip** diuen una part; les preguntes es reparteixen a l'atzar. |
| "Ha quedat molt bé" com a tot argument | La frase obligada: *"la decisió més important va ser…"* |

## El públic també treballa

Qui escolta omple la **coavaluació amb criteris** de la fitxa ampliada (2 estrelles i un desig ancorats a la R4). Escoltar per avaluar és la meitat de l'aprenentatge de comunicar.

## Logística de la SA9 (nota per al docent)

Amb més de **6 equips**, una sola sessió de defenses (S5) no dona: 5' + preguntes + canvi de muntatge ≈ 12-15' per equip. **Esglaona-les:** 2-3 defenses al final de les sessions S3 i S4 (equips que ja tenen el prototip llest) i la resta a la S5. Avantatge afegit: els primers equips reben preguntes que els altres encara poden aprofitar per millorar.
