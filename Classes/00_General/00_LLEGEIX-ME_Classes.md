...
- **Fitxa ampliada** (`SAx_fitxa_ampliada.md`): **versió d'aprofundiment** amb totes les rutines (revisió creuada, exit ticket, ODS, pensament computacional) i ampliacions.
...
- **Qüestionaris de repàs autocorrectius:** [`00_Questionaris_repas.md`](00_Questionaris_repas.md) — un **Google Form autocorrectiu per SA** (8-9 preguntes del nucli, resposta immediata, repetible, no qualifica). És el repàs espaiat **fora de l'aula**: es publica al Classroom en tancar cada SA, i és la repesca natural dels 🔴 del mini-check.
- **Mini-checks individuals:** [`00_Mini_checks_individuals.md`](00_Mini_checks_individuals.md) — un micro-repte de codi de **10' en solitari** per SA (SA2–SA8), amb semàfor de correcció i accions de reforç. No qualifica: és el radar que confirma que cadascú escriu codi tot sol. (El curs treballa **individualment** —vegeu `Programació didàctica/04_Metodologia.md` §4.3—, però el mini-check segueix sent útil com a evidència escrita i per detectar qui va tirant de la còpia del company.)
- **Katas de programació:** un kata d'escriptura de **10' abans d'obrir cada sketch donat** (amb apunts permesos): l'alumnat escriu de zero el bloc central i després compara amb el sketch de la pràctica. Un fitxer `SAn_katas.md` per SA (SA2–SA8); vegeu-ne un exemple a [`../SA2/SA2_katas.md`](../SA2/SA2_katas.md).
...
- **Recurs transversal d'aula:** [`00_Poster_aula_metode_DEPURA_rols.md`](00_Poster_aula_metode_DEPURA_rols.md) (per projectar/imprimir): mètode de projecte, rutina de depuració **DEPURA**, els **quatre barrets** de la feina i autoavaluació/revisió creuada. Les rutines del pòster es concreten a cada fitxa i guia.
...
- **Plantilla de disseny d'objecte:** [`00_Plantilla_disseny_objecte.md`](00_Plantilla_disseny_objecte.md) — full A4 imprimible perquè l'alumnat dissenyi el seu objecte (empatitzar → definir → idear/esbós → prototipar → provar → comunicar) amb autoavaluació i revisió creuada.
