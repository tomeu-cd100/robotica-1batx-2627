...
Fins ara cada placa vivia sola. Aquí en poses **dues a parlar-se sense cables**: sacseges una micro:bit, «llança el dau», i el resultat apareix a l'altra placa. És la primera **comunicació sense fils** del curs, i la fa possible el mòdul `radio` amb una sola idea clau: el **`group`** és el canal — dues plaques només se senten si comparteixen el **mateix número de grup**.
...
Sense executar el codi (a baix, plegat): si sacseges la placa A, què veurà la placa A i què veurà la B? Què passarà si una placa té `group=10` i l'altra `group=7`? I si a l'aula hi ha **tres** persones totes amb `group=10`? Apunta-ho a l'Activitat 3 de la [fitxa](../SA5_fitxa_alumnat.md) i comprova-ho.
...
Així es veu **a la placa real** després d'un sacseig. Però fixa-t'hi: una foto d'una sola placa només explica **mitja pràctica** — la gràcia és a la **segona placa**, que mostrarà `R` + el mateix número **sense cap cable entremig**. La ràdio no surt a les fotos: l'has de veure funcionar.
...
Dues línies que van **fora** del `while True:` perquè només calen **una vegada**, en engegar. `radio.on()` encén l'antena (si te'l deixes, `send` i `receive` no fan res, en silenci). `radio.config(group=10)` tria el **canal**: totes les micro:bit de l'aula comparteixen l'aire, i el `group` (0-255) és el que separa les converses. Regla d'or: **mateix `group` a les teves dues plaques i `group` diferent del dels companys** (per exemple, el número de la teva taula) — si no, rebràs els daus dels veïns.
...
| Reps daus «fantasma» | Un company té el mateix `group`: canvia'l (número de taula). |
| El dau es llança quan camines amb la placa | `"shake"` és sensible: qualsevol sotragada forta compta. Per al comandament del braç triaràs gestos més estables. |
