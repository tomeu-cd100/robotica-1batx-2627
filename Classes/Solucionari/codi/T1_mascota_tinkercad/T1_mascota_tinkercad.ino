// Variant per a TINKERCAD del codi de referencia del Projecte T1 (NOMES DOCENT)
// El fitxer bo per al maquinari real es T1_mascota.ino. Aquesta variant nomes
// canvia el que Tinkercad no suporta:
//   - Sense DHT11: Tinkercad no te ni el component ni la llibreria DHT.
//   - Estats com a const int en lloc d'enum: el preprocessador de Tinkercad
//     genera els prototips ABANS de les definicions de tipus i un enum com a
//     parametre de funcio fa error ("'Emocio' was not declared").
//   - Al simulador: potenciometre a A0 (fa de microfon) i fotoresistencia
//     (LDR) a A1 (fa de TEMT6000). El codi no canvia: analogRead identic.
...
// --- Maquina d'estats de les emocions (const int, no enum: vegeu capcalera) ---
const int CONTENT = 0;
...
void llegeixSensors() {
  ...
  if (digitalRead(PIN_PIR) == HIGH) {
    ...
    tUltimEstimul = millis();  // refresca si ja estem curiosos, deixa que la caricia funcioni
  }
