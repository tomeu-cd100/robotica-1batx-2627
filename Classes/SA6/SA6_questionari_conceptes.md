...
2. La **realimentació** (*feedback*) en un sistema de control consisteix a…
   ...
   - b) **Mesurar la sortida real i tornar-la a l'entrada per comparar-la amb la consigna.**
   - c) Apagar el sistema quan hi ha un error.
...
3. La **consigna** (*setpoint*) és…
   ...
   - c) **El valor desitjat que volem que el sistema assoleixi.**
   - d) La sortida de l'actuador.
...
5. La **histèresi** en un termòstat consisteix a…
   - a) Encendre i apagar amb un únic llindar.
   - b) **Usar dos llindars diferents: un per encendre i un altre per apagar.**
...
6. Si controlem un ventilador amb **un sol llindar** (sense histèresi), a prop de la consigna…
   ...
   - c) **El ventilador s'encén i s'apaga contínuament (parpelleig/oscil·lació).**
   - d) És el comportament ideal, no passa res.
...
9. En el **control proporcional**, l'actuació sobre l'actuador…
   - a) És sempre màxima o mínima (tot o res).
   - b) **És proporcional a l'error: com més gran l'error, més forta la correcció.**
...
10. Si en un control proporcional la constant `Kp` és **massa gran**, el sistema…
    ...
    - b) **Tendeix a oscil·lar (es pot limitar la sortida amb `constrain`).**
    - c) Deixa de necessitar el sensor.
    - d) Es converteix en un llaç obert.
...
2. La realimentació és… → Un sensor de temperatura / **Mesurar la sortida i tornar-la a l'entrada per comparar-la amb la consigna** / Apagar el sistema en error / Accelerar el programa.
3. La consigna (setpoint) és… → El valor que mesura el sensor / L'error / **El valor desitjat que volem assolir** / La sortida de l'actuador.
4. L'error es calcula com… → `consigna * mesura` / **`consigna - mesura`** / `mesura + actuador` / `sensor / consigna`.
5. La histèresi consisteix a… → Un únic llindar / **Dos llindars: un per encendre i un per apagar** / No usar sensor / Pujar la temperatura sense límit.
6. Un sol llindar (sense histèresi) → Queda apagat sempre / El sensor deixa de funcionar / **S'encén i s'apaga contínuament (oscil·lació)** / És l'ideal.
...
9. En el control proporcional l'actuació… → És sempre tot/res / **És proporcional a l'error** / No depèn de l'error / Depèn només del temps.
10. Si `Kp` és massa gran… → Es queda apagat / **Tendeix a oscil·lar (limitar amb `constrain`)** / No cal sensor / Es torna llaç obert.
