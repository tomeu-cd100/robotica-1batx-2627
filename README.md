...
5. **Google Classroom** (opcional): els scripts de `Material Classroom/` demanen un projecte propi a Google Cloud Console (OAuth client → `credentials.json`, mai al repo) i el `config.js` amb els ids del teu curs (obtén els ids de categories amb `node estat_classroom.js`).
   - ⚠️ **Substitueix els enllaços al Classroom original.** Les portades i fitxes de cada SA enllacen les tasques del **curs de l'autor** (`classroom.google.com/c/ODY4ODU4Njk0NTEy/...`), on el teu alumnat no pot entrar. Localitza'ls amb `grep -rn "classroom.google.com" Classes/` i canvia'ls pels teus (o esborra'ls si no fas servir Classroom: la feina també és a les fitxes). És l'única cosa del fork que **no** es reconstrueix sola.
6. **Contrasenya de la vista docent**: la vista docent del web té una porta de contrasenya (fricció perquè l'alumnat no hi entri per curiositat; **no és seguretat**: el material continua sent públic al repositori). Posa-hi la teva amb `py tools/canvia_contrasenya_docent.py` i commiteja el canvi. Perquè la porta no es pugui esquivar pel costat, el generador **no fa PDF de les pàgines docents** (un PDF no executa la porta i la seva URL és endevinable: imprimeix-les des del navegador), les marca amb `noindex` perquè no surtin als cercadors i **no indexa el seu text** a `cerca-index.js` (només el títol).
